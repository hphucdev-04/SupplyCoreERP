...
namespace SupplyCoreERP.Inventories.Balances
{
	public class InventoryBalanceManager : DomainService
	{
			...
			IEnumerable<InventoryTicketDetail> details, Guid warehouseId)
		{
			...
			var balances = await _balanceRepo.GetListAsync(x =>
		...
		{
			var balances = await GetBalancesAsync(details, ticket.WarehouseId);
			var reservations = new List<InventoryReservation>();
			var balancesToUpdate = new List<InventoryBalance>();

			foreach (var item in details)
			{
				if (!balances.TryGetValue((item.BinId, item.ProductBatchId), out var balance) || balance.AvailableQuantity < item.BaseQuantity)
					throw new UserFriendlyException($"Không đủ tồn kho khả dụng cho sản phẩm ID {item.ProductId}!");
		...
		public async Task UnlockStockAsync(InventoryTicket ticket)
		{
			...
			var binIds = activeReservations.Select(x => x.BinId).Distinct().ToList();
			var batchIds = activeReservations.Select(x => x.ProductBatchId).Distinct().ToList();
			...
			{
				res.Cancel();
				var balance = balances.FirstOrDefault(x => x.BinId == res.BinId && x.ProductBatchId == res.ProductBatchId);
				if (balance != null)
				{
					balance.UnlockStock(res.ReservedQuantity);
					if (!balancesToUpdate.Contains(balance)) balancesToUpdate.Add(balance);
				}
			}

			await _reservationRepo.UpdateManyAsync(activeReservations);
			if (balancesToUpdate.Any()) await _balanceRepo.UpdateManyAsync(balancesToUpdate);
		}

		// ĐIỀU CHỈNH LOCK ĐƠN LẺ ĐÃ ĐƯỢC CẬP NHẬT
		public async Task AdjustLockAsync(InventoryTicket ticket, Guid binId, Guid productId, Guid productBatchId, decimal baseQtyDiff)
		{
			...
			if (baseQtyDiff > 0) // Lock thêm
			{
				if (balance == null || balance.AvailableQuantity < baseQtyDiff)
					throw new UserFriendlyException("Không đủ tồn kho khả dụng!");

				balance.LockStock(baseQtyDiff);
				await _balanceRepo.UpdateAsync(balance);

				// Ưu tiên tìm Reservation cũ để TĂNG lên
				var existingRes = await _reservationRepo.FirstOrDefaultAsync(x =>
		...
		public async Task ExecuteStockMovementAsync(InventoryTicket ticket, IEnumerable<InventoryTicketDetail> details, InventoryTransactionType transType, bool isIssue)
		{
			...
			var activeReservations = isIssue
			...
			var balancesToInsert = new List<InventoryBalance>();
			var balancesToUpdate = new List<InventoryBalance>();

			foreach (var item in details)
			{
				balances.TryGetValue((item.BinId, item.ProductBatchId), out var balance);

				if (isIssue)
				{
					if (balance == null) throw new UserFriendlyException("Không có tồn kho!");

					var relatedRes = activeReservations.FirstOrDefault(x => x.BinId == item.BinId && x.ProductBatchId == item.ProductBatchId && x.Status == ReservationStatus.Active);
					if (relatedRes != null)
					{
						relatedRes.Complete();
				...
				else // Nhập kho
				{
					...
					else
					{
						...
						if (!balancesToUpdate.Contains(balance) && !balancesToInsert.Contains(balance)) balancesToUpdate.Add(balance);
					}
				}
			...
			if (activeReservations.Any()) await _reservationRepo.UpdateManyAsync(activeReservations);
			if (balancesToInsert.Any()) await _balanceRepo.InsertManyAsync(balancesToInsert);
			if (balancesToUpdate.Any()) await _balanceRepo.UpdateManyAsync(balancesToUpdate);
			if (transactions.Any()) await _transactionRepo.InsertManyAsync(transactions);
		}
	}
}