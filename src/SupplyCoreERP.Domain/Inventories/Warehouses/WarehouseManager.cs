...
namespace SupplyCoreERP.Inventories.Warehouses
{
	public class WarehouseManager : DomainService
	{
		...
		#region Warehouse
		public async Task<Warehouse> CreateAsync(string code, string name, string? address, Guid? cityId, Guid? areaId, int width, int length)
		{
		...
		public async Task UpdateAsync(Warehouse warehouse, string code, string name, string? address, Guid? cityId, Guid? areaId, int width, int length)
		{
			if (warehouse.Code != code && await _warehouseRepo.AnyAsync(x => x.Code == code))
				throw new UserFriendlyException($"Mã kho '{code}' đã tồn tại!");
		...
		#region Zone
		public async Task<Zone> CreateZoneAsync(Guid warehouseId, string code, string name, ZoneType type, StorageCondition condition, string color, int x, int y, int w, int l, float rotation)
		{
		...
		public async Task UpdateZoneAsync(Zone zone, string code, string name, ZoneType type, StorageCondition condition, string? color, int x, int y, int w, int l, float rotation)
		{
			if (zone.Code != code && await _zoneRepo.AnyAsync(z => z.WarehouseId == zone.WarehouseId && z.Code == code && z.Id != zone.Id))
				throw new UserFriendlyException($"Mã khu vực '{code}' đã tồn tại trong kho này!");
