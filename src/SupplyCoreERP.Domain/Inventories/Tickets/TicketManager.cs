...
namespace SupplyCoreERP.Inventories.Tickets
{
	public class TicketManager : DomainService
	{
		...
		public async Task<InventoryTicket> CreateTicketAsync(TicketType type, Guid warehouseId, Guid? referenceDocumentId, string? referenceDocumentNumber, string? note)
		{
			...
			if (!warehouse.IsActive) throw new UserFriendlyException($"Kho '{warehouse.Name}' đang bị tạm khóa!");
		...
		public async Task ValidateBeforeDeleteAsync(InventoryTicket ticket)
		{
			...
			if (IsIssueTicket(ticket.Type)) await _balanceManager.UnlockStockAsync(ticket);
		}
		#endregion
			...
			InventoryTicket ticket, Guid productId, Guid productBatchId, Guid binId, Guid unitId, int conversionFactor, decimal quantity)
		{
			...
			if (bin.WarehouseId != ticket.WarehouseId) throw new UserFriendlyException("Vị trí (Bin) không thuộc kho của phiếu này!");

			if (IsIncomingTicket(ticket.Type)) await ValidateBinForIncomingAsync(binId, productId, productBatchId);
		...
		{
			if (ticket.Status != ApprovalStatus.Draft) throw new UserFriendlyException("Chỉ gửi duyệt phiếu Nháp!");
