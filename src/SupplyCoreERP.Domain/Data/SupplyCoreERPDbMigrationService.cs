...
public class SupplyCoreERPDbMigrationService : ITransientDependency
{
    public ILogger<SupplyCoreERPDbMigrationService> Logger { get; set; }

    private readonly IDataSeeder _dataSeeder;
    private readonly IEnumerable<ISupplyCoreERPDbSchemaMigrator> _dbSchemaMigrators;
    private readonly ICurrentTenant _currentTenant;

    public SupplyCoreERPDbMigrationService(
        IDataSeeder dataSeeder,
        ICurrentTenant currentTenant,
        IEnumerable<ISupplyCoreERPDbSchemaMigrator> dbSchemaMigrators)
    {
        _dataSeeder = dataSeeder;
        _currentTenant = currentTenant;
        _dbSchemaMigrators = dbSchemaMigrators;

        Logger = NullLogger<SupplyCoreERPDbMigrationService>.Instance;
    }
    ...
    private async Task SeedDataAsync()
    {
        await _dataSeeder.SeedAsync(new DataSeedContext()
            .WithProperty(IdentityDataSeedContributor.AdminEmailPropertyName,
                SupplyCoreERPConsts.AdminEmailDefaultValue)
            .WithProperty(IdentityDataSeedContributor.AdminPasswordPropertyName,
                SupplyCoreERPConsts.AdminPasswordDefaultValue)
        );
    }
