...
public class PurchaseRequisition : FullAuditedAggregateRoot<Guid>
{
    ...
    public void UpdateInfo(Guid warehouseId, DateTime? requiredDate, string? note)
    {
        ...
        {
            throw new BusinessException("SupplyCoreERP:InvalidPurchaseRequisitionStatus", "Chỉ có thể sửa yêu cầu khi đang ở trạng thái Nháp hoặc Bị từ chối.");
        }
    ...
    public PurchaseRequisitionLine AddLine(Guid id, Guid productId, Guid unitId, decimal quantity, string? note)
    {
        if (Status != PurchaseRequisitionStatus.Draft && Status != PurchaseRequisitionStatus.Rejected)
        {
            throw new BusinessException("SupplyCoreERP:InvalidPurchaseRequisitionStatus", "Chỉ có thể thêm sản phẩm khi yêu cầu đang ở trạng thái Nháp hoặc Bị từ chối.");
        }

        PurchaseRequisitionLine line = new(id, Id, productId, unitId, quantity, note);
        Lines.Add(line);
        return line;
    }
    ...
    public void UpdateLine(Guid lineId, decimal quantity, string? note)
    {
        if (Status != PurchaseRequisitionStatus.Draft && Status != PurchaseRequisitionStatus.Rejected)
        {
            throw new BusinessException("SupplyCoreERP:InvalidPurchaseRequisitionStatus", "Chỉ có thể sửa sản phẩm khi yêu cầu đang ở trạng thái Nháp hoặc Bị từ chối.");
        }

        PurchaseRequisitionLine? line = Lines.FirstOrDefault(x => x.Id == lineId);
        if (line == null)
        {
            throw new BusinessException("SupplyCoreERP:PurchaseRequisitionLineNotFound", "Không tìm thấy dòng yêu cầu.");
        }
    ...
    public void RemoveLine(Guid lineId)
    {
        if (Status != PurchaseRequisitionStatus.Draft && Status != PurchaseRequisitionStatus.Rejected)
        {
            throw new BusinessException("SupplyCoreERP:InvalidPurchaseRequisitionStatus", "Chỉ có thể xóa sản phẩm khi yêu cầu đang ở trạng thái Nháp hoặc Bị từ chối.");
        }
    ...
    public void SendToApprove()
    {
        if (!Lines.Any())
        {
            throw new UserFriendlyException("YÃªu cáº§u chÆ°a cÃ³ sáº£n pháº©m nÃ o!");
        }

        if (Status != PurchaseRequisitionStatus.Draft && Status != PurchaseRequisitionStatus.Rejected)
        {
            throw new BusinessException("SupplyCoreERP:InvalidPurchaseRequisitionStatus", "Trạng thái hiện tại không cho phép gửi duyệt.");
        }
    ...
    public void Approve()
    {
        ...
        {
            throw new BusinessException("SupplyCoreERP:InvalidPurchaseRequisitionStatus", "Yêu cầu chưa được gửi duyệt.");
        }
    ...
    public void Reject()
    {
        if (Status != PurchaseRequisitionStatus.PendingApproval)
        {
            throw new BusinessException("SupplyCoreERP:InvalidPurchaseRequisitionStatus", "Chỉ có thể từ chối yêu cầu đang chờ duyệt.");
        }
