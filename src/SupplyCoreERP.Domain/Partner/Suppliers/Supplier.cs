...
public class Supplier : FullAuditedAggregateRoot<Guid>
{
    ...
    public virtual ICollection<SupplierProduct> SupplierProducts { get; private set; } = new List<SupplierProduct>();
    private Supplier() { }
    ...
    public void AddDebt(decimal amount)
    {
        if (amount <= 0)
        {
            throw new BusinessException("SupplyCoreERP:InvalidDebtAmount", "Số tiền ghi nợ phải lớn hơn 0");
        }

        if (DebtLimit > 0 && (CurrentDebt + amount > DebtLimit))
        {
            throw new BusinessException("SupplyCoreERP:ExceedsDebtLimit", $"Vượt quá hạn mức nợ cho phép ({DebtLimit:N0}).");
        }
    ...
    public void PayDebt(decimal amount)
    {
        if (amount <= 0)
        {
            throw new BusinessException("SupplyCoreERP:InvalidPaymentAmount", "Số tiền thanh toán phải lớn hơn 0");
        }
    ...
    public SupplierProduct AddProduct(
        ...
        string? note = null)
    {
        if (SupplierProducts.Any(x => x.ProductId == productId))
        {
            throw new BusinessException("SupplyCoreERP:ProductAlreadyExists", "Sản phẩm này đã có trong danh mục của nhà cung cấp.");
        }
        ...
        string? note)
    {
        SupplierProduct sp = SupplierProducts.FirstOrDefault(x => x.ProductId == productId)
            ?? throw new BusinessException("SupplyCoreERP:ProductNotFound", "Không tìm thấy sản phẩm trong danh mục nhà cung cấp.");
    ...
    public void RemoveProduct(Guid productId)
    {
        ...
        SupplierProducts.Remove(sp);
    }

    public void ToggleProductActive(Guid productId)
    {
        SupplierProduct sp = SupplierProducts.FirstOrDefault(x => x.ProductId == productId)
            ?? throw new BusinessException("SupplyCoreERP:ProductNotFound", "Không tìm thấy sản phẩm trong danh mục nhà cung cấp.");
