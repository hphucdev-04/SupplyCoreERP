...
public class SupplierProduct : AuditedEntity<Guid>
{
    ...
    public void ValidateConditions()
    {
        ...
        foreach (IGrouping<Guid, SupplierProductCondition> group in groups)
        {
            ...
            List<int> distinctFactors = group.Select(x => x.ConversionFactor).Distinct().ToList();
            if (distinctFactors.Count > 1)
            {
                throw new BusinessException("SupplyCoreERP:InconsistentConversionFactors", "Các mức quy cách của cùng một đơn vị tính bắt buộc phải sử dụng chung một hệ số quy đổi!");
            }

            // Sắp xếp các mức số lượng tối thiểu tăng dần
            List<SupplierProductCondition> orderedConditions = group.OrderBy(x => x.MinOrderQuantity).ToList();
            ...
            for (int i = 1; i < orderedConditions.Count; i++)
            {
                ...
                {
                    throw new BusinessException("SupplyCoreERP:DuplicateMinOrderQuantity", $"Đơn vị tính đã được cấu hình mốc số lượng đặt tối thiểu là {current.MinOrderQuantity}!");
                }

                // 3. Quy tắc B: MOQ lớn hơn thì giá phải nhỏ hơn hoặc bằng
                if (current.StandardPrice > prev.StandardPrice)
                {
                    throw new BusinessException("SupplyCoreERP:InconsistentPricing", $"Đơn giá thỏa thuận cho mốc số lượng đặt lớn hơn ({current.MinOrderQuantity}) phải nhỏ hơn hoặc bằng mức giá của mốc số lượng nhỏ hơn ({prev.StandardPrice:N0}đ)!");
                }
            }
        }
    }
