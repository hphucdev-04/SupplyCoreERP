...
public class SupplierManager : DomainService, ISupplierManager
{
    ...
    public async Task DeleteAsync(Guid id)
    {
        ...
        if (supplier.CurrentDebt > 0)
        {
            throw new BusinessException(
                "SupplyCoreERP:CannotDeleteSupplierWithOutstandingDebt",
        ...
        string? note = null)
    {
        ...
        Product product = await _productRepo.FindAsync(productId)
            ?? throw new BusinessException("SupplyCoreERP:ProductNotFound", "Sản phẩm không tồn tại.");

        if (!product.IsAvailableForInventory)
        {
            throw new BusinessException("SupplyCoreERP:ProductNotAvailable", $"Sản phẩm '{product.Name}' chưa được duyệt. Không thể thêm vào danh mục cung cấp.");
        }

        if (!await _unitRepo.AnyAsync(x => x.Id == defaultUnitId))
        {
            throw new BusinessException("SupplyCoreERP:UnitNotFound", "Đơn vị tính không tồn tại.");
        }
    ...
    public async Task RemoveProductAsync(Supplier supplier, Guid productId)
    {
        ...
        {
            throw new BusinessException("SupplyCoreERP:ProductNotFound", "Sản phẩm không tồn tại trên hệ thống.");
        }
    ...
    public void ToggleProductActive(Supplier supplier, Guid productId)
    {
        ...
        if (sp == null)
        {
            throw new BusinessException("SupplyCoreERP:ProductNotFound", "Sản phẩm không thuộc nhà cung cấp này.");
        }
    ...
    public async Task CheckCodeAndNameAsync(string code, string name, Guid? excludeId = null)
    {
        ...
        {
            throw new BusinessException("SupplyCoreERP:SupplierCodeExists", $"Mã nhà cung cấp '{code}' đã tồn tại!");
        }

        if (await _supplierRepository.AnyAsync(x =>
                x.Name == normalizedName && (!excludeId.HasValue || x.Id != excludeId.Value)))
        {
            throw new BusinessException("SupplyCoreERP:SupplierNameExists", $"Tên nhà cung cấp '{name}' đã tồn tại!");
        }
    }
    ...
    private async Task ValidateLocationAsync(Guid? countryId, Guid? cityId, Guid? areaId)
    {
        ...
        {
            throw new BusinessException("SupplyCoreERP:CountryNotFound", "Quốc gia không tồn tại!");
        }
        ...
        if (cityId.HasValue)
        {
            ...
            if (city == null)
            {
                throw new BusinessException("SupplyCoreERP:CityNotFound", "Tỉnh/Thành phố không tồn tại!");
            }

            if (countryId.HasValue && city.CountryId != countryId)
            {
                throw new BusinessException("SupplyCoreERP:CityCountryMismatch", $"Thành phố '{city.Name}' không thuộc quốc gia đã chọn!");
            }
        }
        ...
        if (areaId.HasValue)
        {
            ...
            if (area == null)
            {
                throw new BusinessException("SupplyCoreERP:AreaNotFound", "Khu vực (Quận/Huyện) không tồn tại!");
            }

            if (cityId.HasValue && area.CityId != cityId)
            {
                throw new BusinessException("SupplyCoreERP:AreaCityMismatch", $"Khu vực '{area.Name}' không thuộc Tỉnh/Thành phố đã chọn!");
            }
        }
    }
    #endregion
