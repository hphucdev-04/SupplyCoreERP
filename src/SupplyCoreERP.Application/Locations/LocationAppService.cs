...
public class LocationAppService : SupplyCore, ILocationAppService
{
    ...
    {
        List<Continent> items = await _continentRepository.GetListAsync();
    ...
    {
        List<Country> items = await _countryRepository.GetListAsync(x => x.ContinentId == continentId);
    ...
    {
        List<City> items = await _cityRepository.GetListAsync(x => x.CountryId == countryId);
    ...
    {
        List<City> items = await _cityRepository.GetListAsync();
    ...
    {
        List<Area> items = await _areaRepository.GetListAsync(x => x.CityId == cityId);
