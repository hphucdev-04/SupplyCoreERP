...
export const executeTool = async (name: string, args: any) => {
  ...
  if (name === "get_inventory_balance") {
    ...
    try {
      ...
      const resultText = rows
        .map((r: any) => `Sản phẩm: ${r.ProductName} | Kho: ${r.WarehouseName} | Số lượng tồn: ${Number(r.Quantity).toLocaleString('vi-VN')}`)
        .join("\n");
  ...
  if (name === "query_generic_data") {
    ...
    const whitelist: Record<string, { table: string; searchCols: string[]; selectCols: string[] }> = {
      supplier: {
        table: "AppSuppliers",
    ...
    try {
      ...
      const resultText = rows.map((r: any) => {
