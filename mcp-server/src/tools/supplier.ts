...
const formatSequenceDate = (): string => {
  const now = new Date();
...
const generateSupplierCodeAsync = async (): Promise<string> => {
  const todayStr = formatSequenceDate();
  const sequenceRows = await queryDb(
    `SELECT "Id", "PrefixDate", "LastValue"
  ...
  if (sequenceRows.length === 0) {
    ...
    await queryDb(
      `INSERT INTO "AppDocumentSequences" (
  ...
  await queryDb(
    `UPDATE "AppDocumentSequences"
     SET "PrefixDate" = $1,
         "LastValue" = $2
     WHERE "Id" = $3`,
    [todayStr, nextValue, sequence.Id]
  );

  return `${SUPPLIER_DOCUMENT_TYPE}${todayStr}${nextValue.toString().padStart(4, "0")}`;
};

const validateSupplierLocationAsync = async (
  countryId?: string,
  cityId?: string,
  areaId?: string
): Promise<void> => {
  if (countryId) {
    const countryRows = await queryDb(
      `SELECT "Id" FROM "AppCountries" WHERE "Id" = $1 AND "IsDeleted" = false LIMIT 1`,
      [countryId]
    );
  ...
  if (cityId) {
    const cityRows = await queryDb(
      `SELECT "Id", "CountryId" FROM "AppCities" WHERE "Id" = $1 AND "IsDeleted" = false LIMIT 1`,
      [cityId]
    );
  ...
  if (areaId) {
    const areaRows = await queryDb(
      `SELECT "Id", "CityId" FROM "AppAreas" WHERE "Id" = $1 AND "IsDeleted" = false LIMIT 1`,
      [areaId]
    );
...
const validateSupplierUniquenessAsync = async (code: string, name: string): Promise<void> => {
  const duplicateCodeRows = await queryDb(
    `SELECT "Id" FROM "AppSuppliers" WHERE "Code" = $1 AND "IsDeleted" = false LIMIT 1`,
    [code]
  );
  ...
  const duplicateNameRows = await queryDb(
    `SELECT "Id" FROM "AppSuppliers" WHERE "Name" = $1 AND "IsDeleted" = false LIMIT 1`,
    [name]
  );
