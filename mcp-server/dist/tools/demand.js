...
export const registerDemandTools = (server) => {
    server.registerTool("get_demand_history", {
        ...
        const periodExpression = bucket === "day"
    ...
    server.registerTool("get_monthly_demand_forecast", {
        ...
        WHERE p."IsDeleted" = false
          AND w."IsDeleted" = false
      `;
        const params = [formatDate(historyStart), formatDate(historyEnd)];
        if (warehouseIds && warehouseIds.length > 0) {
            params.push(warehouseIds);
