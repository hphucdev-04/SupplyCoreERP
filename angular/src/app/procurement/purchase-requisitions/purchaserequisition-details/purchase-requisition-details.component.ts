...
export class PurchaseRequisitionDetailsComponent implements OnInit, OnDestroy {
  ...
  goBack() {
    this.router.navigate(['/procurement/purchase-requisitions']);
  }

  goToOrder(id: string) {
    this.router.navigate(['/procurement/purchase-orders/details', id]);
  }
      ...
      .subscribe({
        next: res => {
          ...
          this.routesService.add([
            {
              path: `/procurement/purchase-requisitions/details/${this.requisition.id}`,
              name: this.ROUTE_NAME,
              parentName: '::Menu:Purchase-Requisitions',
